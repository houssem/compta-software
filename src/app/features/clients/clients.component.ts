...
export class ClientsComponent {
  ...
  factureLabel(client: Client): string {
    if (client.factureStatus === 'a-jour') return 'À JOUR'
    if (client.factureStatus === 'retard') {
      return client.facturesCount === 1 ? '1 RETARD' : `${client.facturesCount} RETARDS`
    }
    return `${client.facturesCount} EN ATTENTE`
  }
}
